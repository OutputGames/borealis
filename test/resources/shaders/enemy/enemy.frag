...
uniform vec3 _color;
uniform vec3 _armorColor;
uniform vec3 _weaponColor;
...
void main()
{
   ...
   if (diff*10.0f < 4.0f) {
    ...
    int idx = getShadeIndex(color.rgb);

   switch (idx) {
      case 0:
         color.rgb = color_vec3(_color) * color.r;
         break;
      case 1:
         color.rgb = color_vec3(_armorColor) * color.g;
        break;
      case 2:
         color.rgb = color_vec3(_weaponColor) * color.b;
        break;
   }
